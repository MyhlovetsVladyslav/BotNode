...
async function handlePhotos(msg, chatId, post) {
  if (!msg.photo) {
    await editMessageWithKeyboard(chatId, msg.message_id, 'Будь ласка, надішліть фотографію або натисніть "Завершити додавання фото".', [
      { text: 'Завершити додавання фото', callback_data: 'finish_photos' }
    ]);
    return;
  }
  ...
  if (msg.media_group_id) {
    ...
    post.photoNotificationTimer = setTimeout(async () => {
      if (!post.hasSentInstruction) {
        await editMessageWithKeyboard(chatId, msg.message_id, `Отримано ${post.photos.length} фото. Ви можете надіслати ще фотографії або натисніть кнопку "Завершити додавання фото", щоб перейти до додавання опису.`, [
          { text: 'Завершити додавання фото', callback_data: 'finish_photos' }
        ]);
    ...
    if (action === 'approve') {
      if (post.photos && post.photos.length > 0) {
        ...
        try {
          ...
          await editMessageWithKeyboard(userChatId, query.message.message_id, 'Ваш пост опубліковано в каналі.', [
            { text: 'Продаж книг', callback_data: 'sell_books' },
            { text: 'Пошук', callback_data: 'search_books' }
          ]);
          ...
          await editMessageWithKeyboard(userChatId, query.message.message_id, 'Сталася помилка при публікації вашого поста. Будь ласка, спробуйте ще раз.', [
            { text: 'Продаж книг', callback_data: 'sell_books' },
            { text: 'Пошук', callback_data: 'search_books' }
          ]);
        }
      } else {
        await editMessageWithKeyboard(userChatId, query.message.message_id, 'Помилка: немає фотографій для публікації.', [
          { text: 'Продаж книг', callback_data: 'sell_books' },
          { text: 'Пошук', callback_data: 'search_books' }
        ]);
      }
    } else if (action === 'reject') {
      await editMessageWithKeyboard(userChatId, query.message.message_id, 'Ваш пост було відхилено модератором.', [
        { text: 'Продаж книг', callback_data: 'sell_books' },
        { text: 'Пошук', callback_data: 'search_books' }
      ]);
      post.stage = 'rejected';
...
async function handleDescription(msg, chatId, post) {
  if (post.stage === AWAITING_DESCRIPTION) {
    if (!msg.text) {
      await editMessageWithKeyboard(chatId, msg.message_id, 'Будь ласка, надішліть текстове описання для вашого поста.', []);
      return;
    }
    ...
    await editMessageWithKeyboard(chatId, msg.message_id, 'Ваш пост відправлено на модерацію. Ви можете створити новий пост або виконати пошук.', [
      { text: 'Продаж книг', callback_data: 'sell_books' },
      { text: 'Пошук', callback_data: 'search_books' }
    ]);
  }
...
async function handleReceipt(msg, chatId, post) {
  if (msg.photo) {
    ...
    await editMessageWithKeyboard(chatId, msg.message_id, 'Квитанція отримана. Надішліть фотографії книг.', []);
  } else {
